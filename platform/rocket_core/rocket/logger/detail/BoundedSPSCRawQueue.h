...
struct BoundedSPSCRawQueueDetail {
    ...
    static constexpr auto alignBufferSize(std::size_t value) noexcept -> std::size_t {
        return rocket::detail::align_up(value, kSegmentSize);
    }
...
class BoundedSPSCRawQueueProducer {
  ...
  public:
    ...
    [[nodiscard]] ROCKET_FORCE_INLINE auto isClosed() const noexcept {
        return std::atomic_ref{header_->closed}.load(std::memory_order_acquire);
    }

    /// Close queue
    ROCKET_FORCE_INLINE void close() noexcept {
    ...
    [[nodiscard]] ROCKET_FORCE_INLINE auto prepare(std::size_t size) noexcept -> std::span<std::byte> {
    ...
    ROCKET_FORCE_INLINE void commit() noexcept {
...
class BoundedSPSCRawQueueConsumer {
  ...
  public:
    ...
    [[nodiscard]] ROCKET_FORCE_INLINE auto fetch() noexcept -> std::span<std::byte const> {
    ...
    ROCKET_FORCE_INLINE void consume() noexcept {
...
class BoundedSPSCRawQueueImpl {
  ...
  public:
    ...
    BoundedSPSCRawQueueImpl(std::string_view name, MemorySource const& memorySource = AnonymousMemorySource()) {
        ...
        if (auto storage = rocket::detail::mapFile(file_); !QueueDetail::check(storage.content())) {
    ...
    BoundedSPSCRawQueueImpl(std::string_view name, CreationOptions const& options,
        ...
        std::size_t const capacity = rocket::detail::align_up(options.capacityHint, pageSize);
        ...
        if (auto const fileSize = file_.getFileSize(); fileSize != 0) {
            ...
            QueueDetail::init(rocket::detail::mapFile(file_, capacity).content());
        }
    }
    ...
    [[nodiscard]] ROCKET_FORCE_INLINE auto createProducer() -> Producer {
        if (!operator bool()) {
            throw std::runtime_error{"queue not initialized"};
        }
        return Producer(rocket::detail::mapFile(file_));
    }

    /// Create consumer for the queue. Throws on error.
    [[nodiscard]] ROCKET_FORCE_INLINE auto createConsumer() -> Consumer {
        if (!operator bool()) {
            throw std::runtime_error{"queue not initialized"};
        }
        if (!file_.tryLock()) {
        ...
        return Consumer(rocket::detail::mapFile(file_));
    }
